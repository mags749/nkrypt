...
export default function ChangePassKeyScreen() {
  ...
  return (
    <ChangePassKeyView
      ...
      onPassPhraseChange={(v) => { hook.setPassPhrase(v); }}
      showPassPhrase={hook.showPassPhrase}
      onToggleShowPassPhrase={() => hook.setShowPassPhrase((v) => !v)}
      currentKey={hook.currentKey}
      onCurrentKeyChange={(v) => { hook.setCurrentKey(v); }}
      newKey={hook.newKey}
      onNewKeyChange={(v) => { hook.setNewKey(v); }}
      newKeyConfirm={hook.newKeyConfirm}
      onNewKeyConfirmChange={(v) => { hook.setNewKeyConfirm(v); }}
